...
class _DeadlineCalendarScreenState extends State<DeadlineCalendarScreen> {
  ...
  Map<String, int> _buildStats(List<_MilestoneEntry> entries, DateTime now) {
    final upcoming = entries
        .where((e) => e.deadline != null && !e.deadline!.isBefore(now))
        .length;
    final urgent = entries
        .where((e) =>
            e.deadline != null &&
            !e.deadline!.isBefore(now) &&
            e.deadline!.difference(now).inDays <= 30)
        .length;
    final caseLinked = entries
        .where((e) =>
            e.kind == _MilestoneKind.caseStep ||
            e.kind == _MilestoneKind.document)
        .length;
  ...
  List<_MilestoneEntry> _buildMilestones(
    ...
    for (final scholarship in controller.scholarships) {
      ...
      final deadline = _parseDeadline(controller.resolve(
      ...
      if (isSaved) {
        ...
        for (final step in RoadmapEngine.getSteps()) {
          if (controller.isStepCompleted(scholarship.id, step.type)) {
            continue;
          }
          entries.add(
            _MilestoneEntry(

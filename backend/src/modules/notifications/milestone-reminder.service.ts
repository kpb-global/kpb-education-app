...
export class MilestoneReminderService {
  ...
  async handleDailyMilestoneReminders() {
    ...
    for (const reminder of reminders) {
      const isEnglish = reminder.user.preferredLanguage === 'en';
  ...
  private async collectSavedScholarshipReminders(
    ...
    for (const saved of savedItems) {
      ...
      reminders.push({
        user: saved.user,
        titleFr: days === 0 ? 'Échéance bourse aujourd’hui' : `Bourse à J-${days}`,
        titleEn:
          days === 0
    ...
    for (const item of cases) {
      if (item.scheduledAt) {
        ...
        if (CASE_REMINDER_DAYS.has(days)) {
          reminders.push({
            user: item.user,
            titleFr: days === 0 ? 'Rendez-vous dossier aujourd’hui' : `Dossier à J-${days}`,
            titleEn:
              days === 0
      ...
      for (const task of item.tasks) {
        ...
        reminders.push({
          user: item.user,
          titleFr: days === 0 ? 'Action dossier aujourd’hui' : `Action dossier à J-${days}`,
          titleEn:
            days === 0

...
| App interaction events | Runtime | Firebase Analytics ; **PostHog seulement si le build porte une clé** | Noms d'événements + paramètres non-PII, **sauf le terme de recherche** (`analytics_service.dart:434-437`). Le miroir PostHog est sauté quand `posthogEnabled` est faux (`app_config.dart:87`). |
...
> | Play → App info & performance → **Journaux de plantage** | Collectés, **collecte obligatoire (l'utilisateur ne peut pas la désactiver)** | Aucun `setCrashlyticsCollectionEnabled` dans le dépôt ; collecteurs armés `main.dart:49-54` avant la lecture du consentement `:110`. |
...
| **OneSignal** | US | Push token, external id = `UserProfile.id` (`onesignal_service.dart:61`), **l'adresse e-mail** (`:62-64`), et **4 étiquettes de profil** : `account_type`, `level`, `target_country`, `locale` (`:69` ; charge construite en `app_controller.dart:1908-1918`) | Push notifications **+ segmentation** | Yes | No |
...
| **Mautic** (self-hosted) | KPB VPS (France) | **7 champs, pas seulement l'e-mail** : e-mail, `firstname`/`lastname` découpés depuis `fullName`, `phone`, `mobile` (= numéro WhatsApp), `country` (pays de résidence), `preferred_locale` (`mautic.service.ts:99-115`) | Scholarship newsletter | Yes | No |
...
> Cases qui changent :
> · Play → Data Safety → **Audio → Voice or sound recordings : collecté, non
> partagé pour la pub, finalité « App functionality »** — raison : l'audio quitte
> l'appareil vers le recognizer de la plateforme, l'exemption « on device » qui
> justifiait l'omission n'est pas adossée au code.
...
- **Identifiant publicitaire (section « Contenu de l'application ») : OUI.**
  Raison : le manifeste fusionné porte `com.google.android.gms.permission.AD_ID`,
...
| App info & performance — Crash logs | Yes | No | **OBLIGATOIRE — « les utilisateurs ne peuvent pas désactiver cette collecte »** | App functionality (stabilité) | **La case que ce lot change.** `setCrashlyticsCollectionEnabled` n'existe nulle part dans le dépôt, et les collecteurs sont armés avant toute lecture de consentement (`main.dart:49-54`, consentement relu seulement `:110`) ; les non-fatales partent sans garde (`app_logger.dart:37-45`, `safe_crashlytics.dart:41-60`). Non éphémère : les rapports persistent en console. |

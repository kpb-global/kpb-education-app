...
|-----------------|---------|--------------------------------------------------------------|
| `KPB_AI_TOOLS_ENABLED` | **`false`** in code, **`true` from build 50 on** | **M1** — the CV generator, motivation letters, interview simulator and AI document review (10 entry points across 4 screens). Lot 11 stripped the civil name from Groq prompts and added `AiConsentGuard` on those routes; that guard has been in production since `458d115ebc8d`. Build 49 shipped **without** the flag, so the tools are absent from it — this is a compile-time constant, no server switch can reach a build that has already left. Build 50 passes `--dart-define=KPB_AI_TOOLS_ENABLED=true`, and `scripts/preflight-ios-archive.sh` now refuses an archive that omits it (`EXPECTED_AI_TOOLS`). Guards: [`test/core/masquage_ai_tools_test.dart`](../test/core/masquage_ai_tools_test.dart) and [`test/release/artifact_signing_contract_test.dart`](../test/release/artifact_signing_contract_test.dart). |

...
Dernière lecture du dépôt : 2026-08-15 (lot 9).

## 1. Destinataires tiers (8 + paiements)

| Destinataire | Hôte / preuve | Région | Ce qui part | Finalité |
|---|---|---|---|---|
| **Groq** | `https://api.groq.com/openai/v1/chat/completions` — `backend/src/modules/ai/llm.service.ts` | États-Unis | Texte libre du coach ; profil pseudonymisé (niveau, pays, tranche de budget). Les routes `/tools/*` acceptent encore un champ `name` et le recopient (`tools.service.ts` : `Nom : ${dto.name}`) — masquées côté client par `KPB_AI_TOOLS_ENABLED=false` (lot 7). | Réponses IA |
...
|---|---|---|---|
| Nom, e-mail, téléphone, WhatsApp | Onboarding | Postgres ; e-mail aussi Supabase Auth | Le nom n’est **pas** envoyé à Groq par le coach ; les outils IA masqués le feraient encore côté serveur |
